...
class ResponseAfsw {
	...
	public function ajaxExec( $forceAjax = false ) {
		$isAjax = $this->isAjax();
		$redirect = ReqAfsw::getVar('redirect');
		if (count($this->errors) > 0) {
			$this->error = true;
		}
		if ($isAjax || $forceAjax) {
			HtmlAfsw::echoEscapedHtml(jsonEncodeUTFnormalAfsw($this));
			exit();
		}
		return $this;
	}
	public function mainRedirect( $redirectUrl = '' ) {
		$redirectUrl = empty($redirectUrl) ? AFSW_SITE_URL : $redirectUrl;
		$redirectData = array();
		if (!empty($this->errors)) {
			$redirectData['afswErrors'] = $this->errors;
		}
		if (!empty($this->messages)) {
			$redirectData['afswMsgs'] = $this->messages;
		}
		return redirectAfsw( $redirectUrl . ( strpos($redirectUrl, '?') ? '&' : '?' ) . http_build_query($redirectData) );
	}
	...
	/**
	 * Alias for ResponseAfsw::addError, @see addError method
	 */
