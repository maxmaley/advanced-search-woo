...
class UtilsAfsw {
	...
	public static function jsonEncode( $arr, $ent = false ) {
		return ( is_array($arr) || is_object($arr) ) ? jsonEncodeUTFnormalAfsw($arr, $ent) : jsonEncodeUTFnormalAfsw(array());
	}
	...
	public static function isThisCommercialEdition() {
		foreach (FrameAfsw::_()->getModules() as $m) {
	...
	public static function isMobile() {
		importClassAfsw('Mobile_Detect', AFSW_HELPERS_DIR . 'mobileDetect.php');
	...
	public static function isPluginsPage() {
		return ( basename(ReqAfsw::getVar('SCRIPT_NAME', 'server')) === 'plugins.php' );
	}
	...
	public static function getReferalUrl() {
		// Simple for now
		return ReqAfsw::getVar('HTTP_REFERER', 'server');
	}
	...
	public static function getJSDateFormat( $format = '' ) {
		if (empty($format)) {
			$format = FrameAfsw::_()->getModule('bonuses')->getDateFormat();
		}
	...
	public static function getFormatedDateTime( $dt, $format = '' ) {
		if (empty($format)) {
			$format = FrameAfsw::_()->getModule('bonuses')->getDateFormat() . ' H:i';
		}
	...
	public static function convertDateFormat( $dt, $from = '', $to = 'Y-m-d' ) {
		...
		if (empty($from)) {
			$from = FrameAfsw::_()->getModule('bonuses')->getDateFormat();
		}
		if ($from == $to) {
			return $dt;
		}
