...
abstract class ModuleAfsw extends BaseObjectAfsw {
	...
	public function __construct( $d) {
		...
		if (isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( UtilsAfsw::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( UtilsAfsw::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if (empty($this->_externalDir)) {
			return AFSW_MODULES_DIR . $this->getCode() . DS;
		} else {
	...
	public function getModPath() {
		if (empty($this->_externalPath)) {
			return AFSW_MODULES_PATH . $this->getCode() . '/';
		} else {
	...
	protected function _createController() {
		...
		if (file_exists($this->getModDir() . 'controller.php')) {
			$className = '';
			if (importAfsw($this->getModDir() . 'controller.php')) {
				$className = toeGetClassNameAfsw($this->getCode() . 'Controller');
			}
			if (!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir() . 'helper.php')) {
			...
			importClassAfsw($helper, $this->getModDir() . 'helper.php');
