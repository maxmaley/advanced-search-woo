...
class ErrorsAfsw {
	...
	public static function setSession( $error ) {
		...
		ReqAfsw::setVar('sesErrors', $sesErrors, 'session');
	}
	public static function init() {
		$afswErrors = ReqAfsw::getVar('afswErrors');
		if (!empty($afswErrors)) {
			if (!is_array($afswErrors)) {
				$afswErrors = array( $afswErrors );
			}
			$afswErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $afswErrors)));
			if (!empty($afswErrors)) {
				self::$current = $afswErrors;
				if (is_admin()) {
					add_action('admin_notices', array('ErrorsAfsw', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('ErrorsAfsw', 'appendErrorsContent'), 99999);
				}
			}
		}
	}
	...
	public static function getSession() {
		return ReqAfsw::getVar('sesErrors', 'session');
	}
	public static function clearSession() {
		ReqAfsw::clearVar('sesErrors', 'session');
	}
