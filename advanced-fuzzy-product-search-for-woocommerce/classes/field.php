...
class FieldAfsw {
	 ...
	 */
	public $adapt = array( 'HtmlAfsw' => '', 'dbFrom' => '', 'dbTo' => '' );
	/**
	...
	public function setValue( $value, $fromDB = false ) {
		if (isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB) {
			$value = FieldAdapterAfsw::_($value, $this->adapt['dbFrom'], FieldAdapterAfsw::DB);
		}
		$this->value = $value;
	}
	...
	public function displayValue() {
		...
		switch ($this->html) {
			case 'checkboxlist':
				$options = $this->getHtmlParam('OptionsAfsw');
				$value = array();
				...
				if (empty($value)) {
					$value = esc_html__('N/A', 'advanced-fuzzy-search');
				} else {
				...
				if ('' == $this->value) {
					...
					if (is_array($this->value)) {
						...
						if (!empty($options) && is_array($options)) {
							...
							$value = recImplodeAfsw('<br />', $valArr);
						} else {
							$value = recImplodeAfsw('<br />', $this->value);
						}
					} else {
						$value = $this->value;
					}
	...
	public function showValue() {
		HtmlAfsw::echoEscapedHtml($this->displayValue());
	}
	...
	public function checkVarFromParam( $param, $element ) {
		return UtilsAfsw::xmlAttrToStr($param, $element);
	}
	...
	public function prepareConfigOptions( $xml ) {
		...
		foreach ($config->params->param as $param) {
			...
			$helper = $this->checkVarFromParam($param, 'HelperAfsw');
			$module = $this->checkVarFromParam($param, 'ModuleAfsw');
