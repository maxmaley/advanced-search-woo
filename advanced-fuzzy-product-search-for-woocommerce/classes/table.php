...
abstract class TableAfsw {
	...
	public static function getInstance( $table = '' ) {
		...
		if (!isset($instances[$table])) {
			$class = 'table' . strFirstUpAfsw($table) . strFirstUpAfsw(AFSW_CODE);
	...
	public function isRealTable() {
		return DbAfsw::get("SHOW TABLES LIKE '" . $this->_table . "'");
	}
	public function bulkUpdate( $set, $where ) {
		return DbAfsw::query('UPDATE ' . $this->_table . ' SET ' . $set . ' WHERE ' . $where);
	}
			...
			if ($id) {
				...
				foreach ($data as $field) {
					...
					foreach ($field as $k => $v) {
						if (isset($this->_fields[$k])) {
							$row[$k] = toeCreateObjAfsw('FieldAfsw', array(
									$this->_fields[$k]->name,
									$this->_fields[$k]->html,
									$this->_fields[$k]->type,
	...
	public function getTable( $transform = false ) {
		if ($transform) {
			return DbAfsw::prepareQuery($this->_table);
		} else {
			return $this->_table;
		}
	}
	...
	public function get( $fields = '*', $where = '', $tables = '', $return = 'all' ) {
		...
		return DbAfsw::get($query, $return);
	}
	public function store( $data, $method = 'INSERT', $where = '' ) {
		$this->_clearErrors();
		$method = strtoupper($method);
		if ($this->_escape) {
			$data = DbAfsw::escape($data);
		}
		$query = '';
		switch ($method) {
			case 'INSERT':
		...
		if (empty($fields)) {
			$this->_addError(esc_html__('Nothing to update', 'advanced-fuzzy-search'));
			return false;
		}
		...
		if (DbAfsw::query($query)) {
			if ('INSERT' == $method) {
				return DbAfsw::lastID();
			} else {
				return true;
			}
		} else {
			$this->_addError(AFSW_TEST_MODE ? DbAfsw::getError() : esc_html__('Database error. Please contact your developer.', 'advanced-fuzzy-search'));
		}
		return false;
	}
	public function bulkInsert( $data ) {
		...
		if (empty($data)) {
			$this->_addError(esc_html__('Nothing to insert', 'advanced-fuzzy-search'));
			return false;
		}
		...
		if (!DbAfsw::query(substr($query, 0, -1))) {
			$this->_addError(AFSW_TEST_MODE ? DbAfsw::getError() : esc_html__('Database error. Please contact your developer.', 'advanced-fuzzy-search'));
			return false;
		}
		return true;
			...
			foreach ($data as $k => $v) {
				if (array_key_exists($k, $this->_fields) || $k == $this->_id) {
					...
					if (isset($this->_fields[$k]) && $this->_fields[$k]->adapt['dbTo']) {
						$val = FieldAdapterAfsw::_($val, $this->_fields[$k]->adapt['dbTo'], FieldAdapterAfsw::DB);
					}
					if ($validate) {
						if (isset($this->_fields[$k]) && is_object($this->_fields[$k])) {
							$objForValidation = clone $this->_fields[$k];
							$objForValidation->setValue($val);
							$errors = ValidatorAfsw::_($objForValidation, $this);
							if ($errors) {
	...
	/**
	 * Add new FieldAfswAfsw for children table (@see class field)
	 *
	...
	protected function _addField( $name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '' ) {
		$this->_fields[$name] = toeCreateObjAfsw('FieldAfsw', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
		return $this;
	}
	/**
	...
	public function exists( $value, $field = '' ) {
		...
		return DbAfsw::get('SELECT ' . $this->_id . ' FROM ' . $this->_table . ' WHERE ' . $field . ' = "' . $value . '"', 'one');
	}
			...
			switch ($f->type) {
				case 'date':
					if ('0000-00-00' == $d[$key] || empty($d[$key])) {
						...
						$d[$key] = gmdate(AFSW_DATE_FORMAT, DbAfsw::dateToTime($d[$key]));
					}
					break;
				case 'int':
				case 'tinyint':
	...
	public function getLastInsertID() {
		return DbAfsw::get('SELECT MAX(' . $this->_id . ') FROM ' . $this->_table, 'one');
	}
