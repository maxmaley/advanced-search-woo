 ...
 */
if (!function_exists('strFirstUpAfsw')) {
	function strFirstUpAfsw( $str ) {
 ...
 */
if (!function_exists('dateToTimestampAfsw')) {
	function dateToTimestampAfsw( $date ) {
		if (empty($a)) {
			return false;
		}
		$a = explode(AFSW_DATE_DL, $date);
 ...
 */
if (!function_exists('getRandNameAfsw')) {
	function getRandNameAfsw( $lenFrom = 6, $lenTo = 9 ) {
		$res = '';
...
if (!function_exists('importAfsw')) {
	function importAfsw( $path ) {
		if (file_exists($path)) {
			require($path);
			return true;
		}
...
if (!function_exists('setDefaultParamsAfsw')) {
	function setDefaultParamsAfsw( $params, $default ) {
...
if (!function_exists('importClassAfsw')) {
	function importClassAfsw( $class, $path = '' ) {
		if (!class_exists($class)) {
			if (!$path) {
				$classFile = lcfirst($class);
				if (strpos(strtolower($classFile), AFSW_CODE) !== false) {
					$classFile = preg_replace('/' . AFSW_CODE . '/i', '', $classFile);
				}
				$path = AFSW_CLASSES_DIR . $classFile . '.php';
			}
			return importAfsw($path);
		}
		return false;
	}
}
 ...
 */
if (!function_exists('toeGetClassNameAfsw')) {
	function toeGetClassNameAfsw( $class ) {
		$className = '';
		if (class_exists($class . strFirstUpAfsw(AFSW_CODE))) {
			$className = $class . strFirstUpAfsw(AFSW_CODE);
		} else if (class_exists(AFSW_CLASS_PREFIX . $class)) {
			$className = AFSW_CLASS_PREFIX . $class;
		} else {
			$className = $class;
		}
		return $className;
 ...
 */
if (!function_exists('toeCreateObjAfsw')) {
	function toeCreateObjAfsw( $class, $params ) {
		$className = toeGetClassNameAfsw($class);
		$obj = null;
 ...
 */
if (!function_exists('redirectAfsw')) {
	function redirectAfsw( $url ) {
		if (headers_sent()) {
			if ( class_exists('FrameAfsw') ) {
				FrameAfsw::_()->printInlineJs('document.location.href="' . esc_url($url) . '";');
			}
		} else {
			header('Location: ' . $url);
		}
...
if (!function_exists('jsonEncodeUTFnormalAfsw')) {
	function jsonEncodeUTFnormalAfsw( $value, $ent = false ) {
		if (is_int($value)) {
			return (string) $value;   
		...
		if ($with_keys) {
			foreach ($value as $key => $v) {
				$result[] = jsonEncodeUTFnormalAfsw((string) $key, $ent) . ':' . jsonEncodeUTFnormalAfsw($v, $ent);    
			}
			return '{' . implode(',', $result) . '}';                
		} else {
			foreach ($value as $key => $v) {
				$result[] = jsonEncodeUTFnormalAfsw($v, $ent);    
			}
			return '[' . implode(',', $result) . ']';
		}
	} 
 ...
 * @return array
 */
if (!function_exists('prepareParamsAfsw')) {
	function prepareParamsAfsw( &$d = array(), &$options = array() ) {
		if (!empty($d['params'])) {
			if (isset($d['params']['options'])) {
				$options = $d['params']['options'];
			}
			if (is_array($d['params'])) {
				$params = UtilsAfsw::jsonEncode($d['params']);
		...
		if (isset($d['code'])) {
			if ('' == $d['code']) {
				$d['code'] = prepareFieldCodeAfsw($d['label']) . '_' . rand(0, 9999999);
			}
		}
		return $d;
	}
}
if (!function_exists('prepareFieldCodeAfsw')) {
	function prepareFieldCodeAfsw( $string ) {   
 ...
 */
if (!function_exists('recImplodeAfsw')) {
	function recImplodeAfsw( $glue, $array ) {
		$res = '';
		$i = 0;
		$count = count($array);
		foreach ($array as $el) {
