...
class Fields_ElementorWidgetAfsw extends Widget_Base {
	...
	public function __construct ( $data = array(), $args = null ) {
		...
		$isWooCommercePluginActivated = class_exists('WooCommerce');
		...
		$modPath = FrameAfsw::_()->getModule('fields')->getModPath() . 'assets/';
		
		$isPro = FrameAfsw::_()->isPro();
		
		wp_register_script('afsw-core', AFSW_JS_PATH . 'core.js', array('jquery'), AFSW_VERSION);
		
		wp_register_style('afsw-front-fields', $modPath . 'css/afsw.fields.css', false, AFSW_VERSION);
		wp_register_script('afsw-front-fields', $modPath . 'js/afsw.fields.js', false, AFSW_VERSION);
		if ( \Elementor\Plugin::$instance->preview->is_preview_mode() ) {
			$code = 'var isElementorPreviewAfsw=1;';
			wp_add_inline_script('afsw-front-fields', $code, 'before');
		}
		if ( $isPro ) {
			$modPathPRO = FrameAfsw::_()->getModule('fieldspro')->getModPath() . 'assets/';
			wp_register_style('afsw-front-fields-pro', $modPathPRO . 'css/afsw.fields.pro.css', false, AFSW_VERSION);
			wp_register_script('afsw-front-fields-pro', $modPathPRO . 'js/afsw.fields.pro.js', false, AFSW_VERSION);
		}
	...
	public function get_title() {
		return __( 'Advanced Fuzzy Search', 'advanced-fuzzy-search' );
	}
	...
	public function get_keywords() {
		return array( 'fields', 'afsw', 'fuzzy', 'search', 'woocommerce' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'section_content',
			array(
				'label' => __( 'Select Advanced Fuzzy Field', 'advanced-fuzzy-search' ),
		...
		$this->add_control(
			'field_id',
			array(
				'label' => __( 'Select Field', 'advanced-fuzzy-search' ),
	...
	protected function render() {
		$shortcode = $this->get_settings_for_display( 'field_id' );
		$s = $shortcode ? do_shortcode( '[afsw-fields id="' . $shortcode . '"]' ) : '';
		
		?>
		<div class="elementor-afsw-field"><?php HtmlAfsw::echoEscapedHtml($s); ?></div>
		<?php
	}
